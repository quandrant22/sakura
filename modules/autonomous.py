...
№38: Мониторинг рабочих спринтов — через CPU и время суток детектирует
     ...
     ОТКЛЮЧЕНО ПО РЕШЕНИЮ МАСТЕРА: счётчик считает «работой» открытый
     VS Code или загруженный процессор — без проверки, что человек
     вообще за компьютером (включённый дома ПК тоже считался). Вызов
     из sakura_core/ws_protocol.py убран; update_sprint и её глобальные
     переменные оставлены, чтобы не пришлось восстанавливать.
...
def update_sprint(cpu_percent: float, active_window: str) -> Optional[str]:
    ...
    ВНИМАНИЕ: ОТКЛЮЧЕНО ПО РЕШЕНИЮ МАСТЕРА — вызовы убраны
    из sakura_core/ws_protocol.py. Счётчик считает «работой» открытый
    VS Code или загруженный CPU, не проверяя, что Мастер за компьютером.
    Функция и глобальные переменные (_sprint_start, _sprint_alerted,
    _pause_start) оставлены намеренно, чтобы не пришлось восстанавливать.
    """
