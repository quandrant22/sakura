...
- Ачивки/достижения за период (сегодня/вчера/неделя/месяц/всё время) → {"action": "steam:achievements", "arg": "сегодня|вчера|неделя|месяц|всё"}
- Ачивки/достижения по КОНКРЕТНОЙ игре («какие достижения в X», «достижения по X», «сколько достижений в X», можно с периодом «... за месяц») → {"action": "steam:achievements:game", "arg": "X"} (сводка)
- ПОЛНЫЙ СПИСОК достижений («полный список достижений в X», «покажи все достижения в X», «список ачивок в X») → {"action": "steam:achievements:full", "arg": "X"}
- ТОЛЬКО НЕВЫБИТЫЕ («что осталось выбить в X», «какие не выбиты», «что мне ещё выбить») → {"action": "steam:achievements:todo", "arg": "X"}
- ТОЛЬКО ВЫБИТЫЕ («какие я выбил в X», «что уже выбито», «мои достижения в X») → {"action": "steam:achievements:done", "arg": "X"}
- Какая последняя/крайняя ачивка или достижение, что выбил последнее, какую получил недавно → {"action": "steam:last"}

ПАМЯТЬ:
- «Забудь про Х», «удали из памяти Х» → {"action": "memory:forget", "arg": "Х"}
...
_HARDCODED = [
    ...
    # Музыка — управление: what playing / лайк / дизлайк / next / prev
    #   → music:now_playing / music:like / music:dislike / music:next / music:prev
    #   (deep trigger, через SMTC/хоткеи; браузерный путь — fallback)
    (["что играет", "что сейчас играет", "что у меня играет",
      "какой трек", "какая песня", "что за музыка", "что за трек",
      "что слушаем", "что у меня сейчас играет"],
     {"action": "music:now_playing"}),
    (["лайкни трек", "лайк трек", "добавь в любимые", "нравится трек"],
     {"action": "music:like"}),
    (["дизлайкни трек", "дизлайк трек", "не нравится трек"],
     {"action": "music:dislike"}),
    ...
    # Музыка — лайк/дизлайк (короткие)
    (["дизлайкни", "дизлайк", "не нравится"],
     {"action": "music:dislike"}),
    (["лайкни", "лайк", "нравится", "понравилось"],
     {"action": "music:like"}),
    # Музыка — YM API воспроизведение (deep link → приложение)
    (["мою волну", "моя волна", "волну включи", "включи волну", "включи мою волну"],
          {"action": "music:wave"}),
    # Музыка — shuffle / seek
    (["перемешай", "перемешать", "рандом", "случайн"],
     {"action": "music:shuffle"}),
    (["перемотай вперёд", "вперед музыка"],
     {"action": "music:seek_forward"}),
    (["перемотай назад", "назад музыка"],
     {"action": "music:seek_back"}),
    # Скриншот и зрение
    ...
    (["что у меня на экране", "что на экране", "посмотри на экран",
     ...
     {"action": "screenshot:describe"}),
        # Громкость (разграничение системная vs музыкальная — в _info_hardcoded)
    (["выключи звук", "тишина", "заглуши звук"],
     {"action": "music:mute"}),
    # Браузер
    ...
    # Закрытие приложений (хардкод, границы слов — не перехватывает "закрой вкладку")
    (["закрой браузер", "закрой оперу", "закрой opera", "закрой chrome", "гугл хром"],
     {"action": "close_window:браузер"}),
]

# Паттерны для закрытия приложений с динамическим именем
# Формат: "закрой <имя>", "закрой окно <имя>", "закрой приложение <имя>"
# Используется в _close_app_patterns() для извлечения имени

_CLOSE_APP_PATTERNS = [
    re.compile(r"(?<!\w)закрой\s+(?:окно|приложение|программу?)?\s*(.+)", re.IGNORECASE),
]
...
_PERIOD_RE = re.compile(
    r"(?<!\w)(вчера|сегодня|на этой неделе|за неделю|за 7 дней|за день|за месяц|"
    r"в этом месяце|за 30 дней|за всё время|за все время|вообще|всё|"
    r"всего|недел\w+|месяц\w+|7 дней|30 дней)")

def _period_arg(tl: str) -> str:
    """Вытаскивает каноничный период (ШЕСТЬ вариантов) из фразы.
    сегодня / вчера / неделя / месяц / всё / последняя. Нет периода → неделя (default)."""
    # «последнее достижение», «какую получил последней/крайней/недавно»
    if re.search(r"(?<!\w)(?:последн|крайн|недавно)\w*", tl):
        return "последняя"
    ...
    if p == "вчера":
        return "вчера"
    if p in ("сегодня", "за день"):
        return "сегодня"
    if "месяц" in p or "30 дней" in p:
        return "месяц"
    if p in ("за всё время", "за все время", "вообще", "всё", "всего"):
        return "всё"
    return "неделя"
...
def _info_hardcoded(tl: str) -> dict | None:
    ...
    Музыкальные команды (what playing, like, dislike, next, prev,
    shuffle, repeat, seek, volume, mute, wave) — в _HARDCODED (выше приоритет).
    Здесь только: системная/музыкальная громкость (разграничение), mute.
    """
    # Системная vs музыкальная громкость — РАЗГРАНИЧЕНИЕ
    if re.search(r"(?<!\\w)(?:громче|сделай\\s+громче|прибавь\\s+громкость)", tl):
        if "музык" in tl or "фон" in tl:
            return {"action": "music:volume_up"}      # музыкальная
        return {"action": "volume_up:20"}              # системная
    if re.search(r"(?<!\\w)(?:тише|сделай\\s+тише|убавь\\s+громкость)", tl):
        if "музык" in tl or "фон" in tl:
            return {"action": "music:volume_down"}     # музыкальная
        return {"action": "volume_down:20"}            # системная
    if re.search(r"(?<!\\w)(?:выключи\\s+звук|пауза\\s+звук|тишина|заглуши)", tl):
        return {"action": "music:mute"}

    # Забывание: «забудь про Х», «удали из памяти Х» (с подтверждением).
    # «удали/сотри» — только с «памяти», иначе перехватит удаление файлов/задач.
    # «не забудь» — это напоминание, НЕ забывание.
    m = re.search(r"(?<!\w)забуд\w*\s+(?:про\s+|об\s+|о\s+)?(.+)", tl)
    if m:
        before = tl[:m.start()].rstrip()
        if before.endswith(" не") or before == "не":
            m = None
    if not m:
        m = re.search(r"(?<!\w)(?:удали|сотри)\w*\s+из\s+памяти\s+(.+)", tl)
    if m and m.group(1).strip():
        return {"action": "memory:forget", "arg": m.group(1).strip()}

    # Steam: ачивки / достижения — сводка, полный список, невыбитые, выбитые.
    # (Покрываются и формулировки без слова «ачивки»: «что осталось выбить»,
    #  «что уже выбито в X», «какие я выбил в X».)
    _ach_mod = None
    if re.search(r"(?<!\w)(?:что\s+остал\w*|остал\w*\s+выбить|не\s+выбит\w*"
                 r"|невыбит\w*|ещё\s+выбить|что\s+мне\s+ещё\s+выбить)", tl):
        _ach_mod = "todo"
    if re.search(r"(?<!\w)(?:уже\s+выби)\w*|(?:какие|что)\s+(?:я\s+)?выбил\w*"
                 r"|выбитые\s*", tl):
        _ach_mod = "done"
    if re.search(r"(?<!\w)(?:полн\w*\s+список|покажи\s+все|все\s+ачив\w*"
                 r"|все\s+достижен\w*|список\s+ачив\w*|список\s+достижен\w*)", tl):
        _ach_mod = "full"

    # Игра после в/во/по («какие достижения в Remnant», «осталось выбить в X»).
    # Период может сочетаться («достижения в Remnant за месяц») — он остаётся
    # в исходном тексте и учитывается обработчиком.
    _ach_game = None
    _ag = re.search(r"(?<!\w)(?:в|во|по)\s+(?:игр\w+\s+)?(.+?)\s*[?.!]?$", tl)
    if _ag:
        _g = _PERIOD_RE.sub("", _ag.group(1)).strip(" ,.-")
        _g = re.sub(r"^(?:мои|моих|все|всё|какие|сколько|я|которые)\s+", "", _g).strip()
        if _g and _g.split()[0] not in (
                "игре", "игру", "игра", "игры", "этой", "этом", "прошлой"):
            _ach_game = _g

    if _ach_mod and _ach_game:
        return {"action": f"steam:achievements:{_ach_mod}", "arg": _ach_game}

    # Классические «ачивки/достижения» («достижен» — синоним, все падежи).
    if re.search(r"(?<!\w)(?:ачив|достижен)\w*", tl):
        if re.search(r"прогресс|\bиз\s+\d+", tl):
            return None  # «прогресс по ачивкам» — пусть LLM вытащит игру
        if _ach_game:
            return {"action": "steam:achievements:game", "arg": _ach_game}
...
# ── «Что играет»: устойчивый к искажениям STT матч ───────────────────
# Хардкод выше требует ТОЧНУЮ форму («какой трек»), а Vosk ломает
# падежи: «Какой» → «Какое», «играет» → «играет/играит». Реальный кейс:
# «Какое трек у меня сейчас играет» не матчился и уходил в поиск.
# Пары «существительное + играет» прощают искажения, лишние и
# пропущенные слова — порядок и падежи не важны.
_NOW_PLAYING_PAIRS = (
    (r"трек\w*", r"игра\w*"),       # «какой/какое трек … играет/играит»
    (r"песн\w*", r"игра\w*"),       # «какая песня играет»
    (r"музык\w*", r"игра\w*"),      # «что за музыка играет»
    (r"что", r"игра\w*"),           # «что (у меня/сейчас) играет»
    (r"сейчас", r"игра\w*"),        # «что сейчас играет»
)


def _now_playing_fuzzy(tl: str) -> bool:
    """Пара «существительное + играет» в любом порядке/падеже.

    Исключает «во что играю/играешь» (Steam: steam:current выше
    приоритетом — этот фолбэк его не задевает, т.к. проверяется ПОСЛЕ
    _HARDCODED и _info_hardcoded; здесь же отсекаем формы 1/2 лица,
    чтобы «во что ты играешь» не стало музыкой).
    """
    for key_re, state_re in _NOW_PLAYING_PAIRS:
        if re.search(rf"(?<!\w){key_re}", tl) and re.search(rf"(?<!\w){state_re}", tl):
            # «играю/играешь/играем/играете» — это про игры, не музыку
            if re.search(r"(?<!\w)игра(?:ю|ешь|ем|ете)\b", tl):
                return False
            return True
    return False
...
def _hardcoded_match(text: str) -> dict | None:
    ...
    # Закрытие приложений: "закрой <имя>", "закрой окно <имя>", "закрой приложение <имя>"
    for pattern in _CLOSE_APP_PATTERNS:
        m = pattern.search(tl_clean)
        if m:
            app_name = m.group(1).strip()
            if app_name:
                return {"action": "close_window", "arg": app_name}
    ...
    # «Что играет» при искажениях STT (падежи/лишние слова сломаны):
    # «Какое трек у меня сейчас играет» → music:now_playing.
    # Проверяется ПОСЛЕ точных правил, чтобы steam:current
    # («во что я сейчас играю») и остальные не задевались.
    if _now_playing_fuzzy(tl_clean):
        return {"action": "music:now_playing"}
