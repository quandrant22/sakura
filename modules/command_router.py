...
- Ачивки/достижения по КОНКРЕТНОЙ игре («какие достижения в X», «мои ачивки в X», «достижения по X», «сколько достижений в X», можно с периодом «... за месяц») → {"action": "steam:achievements:game", "arg": "X"}
...
def _info_hardcoded(tl: str) -> dict | None:
    ...
    # Steam: ачивки / достижения («достижен» — синоним, все падежи)
    if re.search(r"(?<!\w)(?:ачив|достижен)\w*", tl):
        ...
        # По конкретной игре: «какие достижения в Remnant», «мои ачивки в Palworld»,
        # «достижения по Fallout Shelter», «сколько достижений в X».
        # Период может сочетаться («достижения в Remnant за месяц») — он остаётся
        # в исходном тексте и учитывается обработчиком.
        g = re.search(r"(?<!\w)(?:в|во|по)\s+(?:игр\w+\s+)?(.+?)\s*[?.!]?$", tl)
        if g:
            game = _PERIOD_RE.sub("", g.group(1)).strip(" ,.-")
            # «мои достижения в игре» — игры нет, это общий вопрос по периоду
            game = re.sub(r"^(?:мои|моих|все|всё|какие|сколько)\s+", "", game).strip()
            if game and game.split()[0] not in (
                    "игре", "игру", "игра", "игры", "этой", "этом", "прошлой"):
                return {"action": "steam:achievements:game", "arg": game}
